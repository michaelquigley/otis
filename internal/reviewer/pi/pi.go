...
func (r *Reviewer) Review(ctx context.Context, req reviewer.Request) (reviewer.Result, error) {
	...
	if req.WorkingDir == "" {
		return reviewer.Result{}, errors.New("pi reviewer working directory is required")
	}
	if len(req.Schema) == 0 {
		return reviewer.Result{}, errors.New("pi reviewer schema is required")
	}
	args := r.args(req.Prompt, req.Model)
	if r.options.DryRun {
	...
	if err != nil {
		return reviewer.Result{}, fmt.Errorf("parse pi output: %w%s", err, reviewer.CommandOutputSuffix(stdout, stderr))
	}
...
func (r *Reviewer) run(ctx context.Context, workingDir string, args []string) ([]byte, []byte, error) {
	...
	if err := cmd.Run(); err != nil {
		return stdout.Bytes(), stderr.Bytes(), fmt.Errorf("pi reviewer failed: %w%s", err, reviewer.CommandOutputSuffix(stdout.Bytes(), stderr.Bytes()))
	}
	return stdout.Bytes(), stderr.Bytes(), nil
}

func (r *Reviewer) args(promptText string, model string) []string {
	args := []string{
